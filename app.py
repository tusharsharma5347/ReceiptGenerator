...
def generate_pdf(row, idx):
    ...
    # Save PDF
    receipt_path = os.path.join(RECEIPT_DIR, f"{serial_no}.pdf")
        ...
        try:
            ...
            if st.button("🖨️ Generate Receipts"):
                ...
                for i, row in to_generate.iterrows():
                    path = generate_pdf(row, i)
                    paths.append(path)
                st.success(f"✅ Generated {len(paths)} receipts in '{RECEIPT_DIR}' folder.")
                for p in paths:
                    st.write(f"- {p}")
